...
func runInit(root string) error {
	...
	// Interactive Q&A
	fmt.Printf("Found %d files. Labels: variables=%d, outputs=%d, modules=%d, data=%d, resources=%d\n",
		len(files),
		len(cts.namesByKind["variable"]),
		len(cts.namesByKind["output"]),
		len(cts.namesByKind["module"]),
		len(cts.namesByKind["data"]),
		len(cts.namesByKind["resource"]))
	...
	candIgnores := topFrequent(append(append(cts.namesByKind["variable"], cts.namesByKind["resource"]...), cts.namesByKind["data"]...), 5)
	...
	// Render file
	content := renderConfig(varPattern, outPattern, modPattern, resPattern, resPattern, ignoreExact, addExperimental)
...
func renderConfig(varPattern, outPattern, modPattern, resPattern, dataPattern string, ignoreExact []string, addExperimental bool) string {
